  ...
  const
{
  ...
  else
  {
    ...
    // Claim the optional measurement time interfaces. In ALL mode they are already included, so
    // this is only needed here in INDIVIDUAL mode.
  ...
  const rclcpp::Time & time, const rclcpp::Duration & /*period*/)
{
  ...
  {
    dynamic_joint_state_msg_.header.stamp = stamp;
    for (auto ji = 0u; ji < dynamic_joint_state_msg_.joint_names.size(); ++ji)
    {
      for (auto ii = 0u; ii < dynamic_joint_state_msg_.interface_values[ji].interface_names.size();
           ++ii)
